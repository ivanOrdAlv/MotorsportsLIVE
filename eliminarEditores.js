...
import supabaseClient from "./services/supabase.js"; // ✅ named import
...
async function cargarEditores() {
  try {
    ...
    document.getElementById("editoresTableBody").classList.add("d-none");
    const editores = await editorService.obtenerEditores();
    console.log("Editores cargados: ", editores);
    const result = await editorService.obtenerEditores();

    if (result.success) {
      console.log("Editores:", result.editores);
    ...
    document.getElementById("editoresTableBody").classList.remove("d-none");
  }
}
...
function mostrarEditores(editores) {
  ...
  editores.forEach((editor) => {
    ...
    tr.innerHTML = `
      ...
      <td>${editor.CorreoElectronico}</td>
      <td>${new Date(editor.created_at).toLocaleDateString()}</td>
      <td>
        <a href="/HTML/admin/confirmarEditor.html?id=${
          editor.id
